...
return f'''<header class="site-header"> <a class="brand" href="{BASE_PATH}/he/"> <span class="brand-title">{SITE_NAME}</span> <span class="brand-subtitle">{SITE_SUBTITLE}</span> </a> <button class="menu-toggle" type="button" aria-expanded="false" aria-controls="main-nav">תפריט</button>
...
return f'''<section class="lesson-signup" aria-label="קבלת עדכונים"> <strong>רוצים לקבל עדכון כשעולה שיעור חדש?</strong> <span>הצטרפו לערוץ הווטסאפ השקט של גמרא למתחילים בניחותא. נשלח שם רק עדכונים קצרים וקישור לשיעור באתר.</span> <a class="button secondary" href="{WHATSAPP_CHANNEL_URL}" target="_blank" rel="noopener">קבלו עדכון בווטסאפ</a>
...
<span class="lesson-number">{number}</span> <span class="lesson-title">{title}</span> <span class="lesson-desc">{desc}</span> </a>'''
)
